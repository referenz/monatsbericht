...
describe('Test der mit den Projektlisten arbeitenden Funktionen', () => {
  ...
  test('Monatsbericht.get_projekte()', () => {
    expect(bericht.getProjekte()).toStrictEqual(projektliste);
  });

  test('Monatsbericht.getProjektData()', () => {
    expect(bericht.getProjektData('01A01K', 'Projekttitel')).toBe('Stadt A für mehr Vielfalt');
    expect(bericht.getProjektData('01A01K', 'Zuwendung 2021')).toBe(100000);
    expect((bericht.getProjektData('01A01K', 'Projektlaufzeit') as string[])[1]).toBe('31.12.2024');
    expect(bericht.getProjektData('99Y99X', 'Träger')).toBe(undefined);
    expect(bericht.getProjektData('01A01K', 'XYZ')).toBe(undefined);
  });

  test('Monatsbericht.getGeendeteProjekte()', () => {
    expect(bericht.getGeendeteProjekte()).toStrictEqual(['01B01K']);
  });

  test('Monatsbericht.getHandlungsbereicheMitProjekten()', () => {
    let result = bericht.getHandlungsbereicheMitProjekten();
    ...
    result = bericht.getHandlungsbereicheMitProjekten(true);
  ...
  test('Monatsbericht.abweichungProjektzahlNachHandlungsbereichen()', () => {
    const [neue_projekte, alte_projekte] = bericht.abweichungProjektzahlNachHandlungsbereichen(bericht_alt);
  ...
  test('Monatsbericht.abweichungProjektdatenNachHandlungsbereichen()', () => {
    // Bezeichnungsabweichungen
    let abweichungen = bericht.abweichungProjektdatenNachHandlungsbereichen(bericht_alt, 'Bezeichnungen');
    ...
    abweichungen = bericht.abweichungProjektdatenNachHandlungsbereichen(bericht_alt, 'Zuwendungen');
